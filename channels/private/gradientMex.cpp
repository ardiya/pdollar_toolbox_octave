...
mxArray* mxCreateMatrix3( int h, int w, int d, mxClassID id, bool c, void **I ){
  const long int dims[3]={h,w,d}, n=h*w*d; int b; mxArray* M; // fix ‘const int*’ to ‘const mwSize* {aka const long int*}’
...
{
  const long int *dims; int nDims; // fix cannot convert ‘const mwSize* {aka const long int*}’ to ‘const int*’
